...
const float upsamplingFactor = 10.;
class TTreeReader;

template <typename TH>
class PrototypeOversampledTH : public ROOT::Detail::RDF::RActionImpl<PrototypeOversampledTH<TH>> {
public:
...
public:
  ...
  PrototypeOversampledTH(std::string_view name, std::string_view title, int nbin, double xmin, double xmax)
  ...
  PrototypeOversampledTH(PrototypeOversampledTH &&) = default;
  PrototypeOversampledTH(const PrototypeOversampledTH &) = delete;
  ...
  std::string GetActionName() { return "PrototypeOversampledTH"; }
};
